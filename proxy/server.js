            ...
            <div id="orders-content" class="tab-content">
                 ...
                 <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
                       ...
                       <div class="lg:col-span-2 bg-white p-6 rounded-2xl shadow-lg">
                           ...
                           <!-- MODIFICATION: Ajout champ heure -->
                           <div class="grid grid-cols-2 gap-4 mb-4">
                               ...
                               <div>
                                   <label for="sale-time" class="block text-sm font-medium text-gray-700">Heure</label>
                                   <input type="time" id="sale-time" class="mt-1 block w-full input-style">
                               </div>
                           </div>
                           <!-- FIN MODIFICATION -->
    ...
    <script type="module">
        ...
        onAuthStateChanged(auth, async (user) => {
            if (user) {
                ...
                // Set default date and time for new sale
                document.getElementById('sale-date').value = getTodayDateString();
                document.getElementById('sale-time').value = getCurrentTimeString();
        ...
        // MODIFICATION: Helper pour l'heure actuelle
        ...
        // MODIFICATION: Helper pour formater date ET heure
        const formatDateTimeToDDMMYYYYHHMM = (date) => {
        ...
        // Helper simple pour juste la date
        ...
        // MODIFICATION: Affichage heure historique
        const renderOrderHistory = (history) => { const container = document.getElementById('order-history-list'); const placeholder = document.getElementById('order-history-placeholder'); container.innerHTML = ''; if (history.length === 0) { container.innerHTML = ''; container.appendChild(placeholder); } else { history.forEach(order => { const card = document.createElement('div'); card.className = 'bg-white p-4 rounded-xl shadow-md'; const displayDateTime = formatDateTimeToDDMMYYYYHHMM(order.saleDate || order.createdAt); const itemsHtml = order.items.map(item => `<li>${item.quantity} x ${item.name}</li>`).join(''); card.innerHTML = `<div class="flex justify-between items-start"><div><p class="font-bold text-gray-700">Vente du ${displayDateTime}</p><ul class="list-disc list-inside text-sm text-gray-600 mt-2">${itemsHtml}</ul></div><div class="flex items-center gap-4 text-right"><p class="font-bold text-lg text-gray-800">${order.totalPrice.toFixed(2)} Mad</p><button data-id="${order.id}" class="delete-order-btn text-gray-400 hover:text-red-600 transition-colors"><i class="fas fa-trash-alt"></i></button></div></div>`; container.appendChild(card); }); } };
        ...
        document.getElementById('product-form').addEventListener('submit', async e => {
            ...
            if (hasRecipe) {
                ...
                if (data.hasOwnProperty('linkedIngredientId')) delete data.linkedIngredientId;
                if (data.hasOwnProperty('linkedIngredientQuantity')) delete data.linkedIngredientQuantity;
            } else {
                ...
                if (isNaN(linkedQty) || linkedQty <= 0) return showToast("Veuillez entrer une quantité consommée valide (positive).");
                data.linkedIngredientId = linkedId;
                data.linkedIngredientQuantity = linkedQty;
                if (data.hasOwnProperty('recipe')) delete data.recipe;
            }

            try {
                if (id) await updateDoc(doc(productsCol, id), data);
                else await addDoc(productsCol, {...data, createdAt: serverTimestamp()});
                showToast(id ? 'Produit mis à jour !' : 'Produit créé !');
                closeModal('product-modal');
            } catch(error) { console.error(error); showToast("Erreur lors de l'opération."); }
        });
        ...
        // MODIFICATION: Enregistrement vente avec heure
        document.getElementById('process-order-btn').addEventListener('click', async () => {
            ...
            if (isNaN(totalPrice) || totalPrice < 0) return showToast("Le montant total de la vente est invalide.");

            const saleDateInput = document.getElementById('sale-date').value;
            const saleTimeInput = document.getElementById('sale-time').value;

            if (!saleDateInput || !saleTimeInput) return showToast("Veuillez sélectionner une date et une heure pour la vente.");

            const saleDateTime = new Date(`${saleDateInput}T${saleTimeInput}:00`);
             if (isNaN(saleDateTime.getTime())) return showToast("Date ou heure invalide.");
            ...
            for (const orderItem of currentOrder) {
                ...
                if (!product) return showToast(`Produit non trouvé : ${orderItem.name}`);
            ...
            for (const [ingId, qty] of tempDeductions.entries()) {
                ...
                if (!ing || ing.quantity < qty) return showToast(`Stock insuffisant pour: ${ing ? ing.name : 'inconnu'}`);
            ...
            batch.set(doc(ordersCol), {
                ...
                createdAt: serverTimestamp(), // Heure d'enregistrement
                saleDate: saleDateTime // Date ET Heure de la vente
            });
        ...
        // MODIFICATION: Filtre historique ne filtre que par date (pas heure)
        const handleDateFilter = () => {
            const filterValue = document.getElementById('history-date-filter').value;
            if (!filterValue) {
                renderOrderHistory(orderHistory); // Show all if no date selected
                return;
            }

            // Create Date objects for start and end of the selected day in local time
            const filterDateStart = new Date(filterValue);
            filterDateStart.setHours(0, 0, 0, 0);

            const filterDateEnd = new Date(filterValue);
            ...
            const filtered = orderHistory.filter(order => {
                ...
                return orderDate.getTime() >= filterDateStart.getTime() && orderDate.getTime() <= filterDateEnd.getTime();
            });
        ...
        document.getElementById('add-po-item-btn').addEventListener('click', () => {
            ...
            currentPurchaseOrderItems.push({
                id: ingredient.id, name: ingredient.name, unit: ingredient.unit, quantity, price: pricePerUnit
            });
        ...
        document.getElementById('purchase-order-form').addEventListener('submit', async (e) => {
            ...
            if (!supplierId || currentPurchaseOrderItems.length === 0) return showToast('Fournisseur et articles requis.');

            const totalCost = parseFloat(document.getElementById('po-total-cost-input').value);
            if (isNaN(totalCost) || totalCost < 0) return showToast("Le total de la commande est invalide.");
            ...
            if (existingPo && existingPo.invoiceUrl && !file) invoiceUrl = existingPo.invoiceUrl;

            const saveOrderData = async (url) => {
                try {
                    const data = { supplierId, items: currentPurchaseOrderItems, totalCost, invoiceUrl: url, paymentStatus };
                    if (poId) {
                        if (existingPo) {
                            data.status = existingPo.status || 'pending';
                            data.createdAt = existingPo.createdAt || serverTimestamp();
                            if (existingPo.receivedAt) data.receivedAt = existingPo.receivedAt;
                            if (paymentStatus === 'paid' && !existingPo.paidAt) data.paidAt = serverTimestamp();
                            else if (existingPo.paidAt) data.paidAt = existingPo.paidAt;
                        }
                        ...
                        if (paymentStatus === 'paid') data.paidAt = serverTimestamp();
                        await addDoc(purchaseOrdersCol, data);
                        showToast('Commande fournisseur créée');
                    }
                    closeModal('purchase-order-modal');
                } catch (dbError) { console.error("Error saving PO:", dbError); showToast(`Erreur DB: ${dbError.message}`);
            ...
            if (file) {
                ...
                } catch (uploadError) { console.error("Error uploading file:", uploadError); showToast(`Erreur Upload: ${uploadError.message}`); fileInput.value = ''; }
            } else { await saveOrderData(invoiceUrl); }
        });
        ...
        document.getElementById('purchase-orders-list').addEventListener('click', async e => {
            ...
            if (target.classList.contains('receive-po-btn')) {
                ...
                purchaseOrder.items.forEach(item => {
                    ...
                    if (quantityToAdd > 0) batch.update(ingRef, { quantity: increment(quantityToAdd) });
                    else console.warn(`Invalid quantity for ${item.name} in PO ${poId}.`);
                });
                ...
                if (confirm('Voulez-vous vraiment supprimer cette commande fournisseur ?')) {
                    try {
                         if (poToDelete && poToDelete.invoiceUrl) {
                             try { await deleteObject(ref(storage, poToDelete.invoiceUrl)); }
                             catch (storageError) { console.error("Error deleting invoice:", storageError); }
                         }
                        await deleteDoc(doc(purchaseOrdersCol, poId));
                        showToast('Commande fournisseur supprimée.');
                    } catch (error) { console.error("Error deleting PO:", error); showToast('Erreur suppression.'); }
                }
                ...
                currentPurchaseOrderItems = purchaseOrder.items.map(item => ({...item, price: Number(item.price) || 0 }));
                renderCurrentPurchaseOrderItems();
                ...
                openModal('purchase-order-modal');
                 ...
                 document.querySelector('input[name="po-payment-status"][value="unpaid"]').checked = true;
                 currentPurchaseOrderItems = poToReorder.items.map(item => ({...item, price: Number(item.price) || 0 }));
                 renderCurrentPurchaseOrderItems();
                 document.getElementById('po-invoice-file').value = '';
                 openModal('purchase-order-modal');
            } else if (target.classList.contains('pay-po-btn')) {
                if (confirm("Voulez-vous marquer cette commande comme 'Payé' ?")) {
                    try { await updateDoc(doc(purchaseOrdersCol, poId), { paymentStatus: 'paid', paidAt: serverTimestamp() }); showToast('Commande marquée comme payée !'); }
                    catch (error) { console.error("Erreur maj paiement:", error); showToast('Erreur maj.'); }
                }
            }
        });
       ...
       // --- IMPORT API SALES (Using Proxy for Auth) ---
        ...
        document.getElementById('api-import-form').addEventListener('submit', async (e) => {
            ...
            if (!startDate || !endDate) return showToast("Veuillez sélectionner les dates.");
            if (!PROXY_URL || PROXY_URL === "https://votre-proxy-url-ici.onrender.com") return showToast("Erreur: URL du PROXY_URL non configurée.");

            loader.classList.remove('hidden');
            statusText.textContent = 'Authentification...';
            submitButton.disabled = true;
            let salesImportedCount = 0, salesSkippedCount = 0, allSalesData = [];

            try {
                // Auth
                const authResponse = await fetch(`${PROXY_URL}/api/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' } });
                const authData = await authResponse.json();
                if (!authResponse.ok) throw new Error(`Échec Auth Proxy: ${authData?.message || authResponse.status}`);
                const token = authData?.token;
                if (!token) throw new Error("Jeton non reçu du proxy.");
                statusText.textContent = 'Récupération ventes...';

                // Fetch Sales
                let currentPage = 1, pageSize = 100, hasMorePages = true;
                while(hasMorePages) {
                    ...
                    if (!salesResponse.ok) throw new Error(`Erreur Proxy Ventes (P${currentPage}): ${salesResult?.message || salesResponse.status}`);
                    if (salesResult.code !== 200 || !salesResult.data) throw new Error(`Erreur Proxy Ventes (P${currentPage}): ${salesResult.msg || 'Données invalides'}`);
                    allSalesData = allSalesData.concat(salesResult.data);
                    if (!salesResult.data || salesResult.data.length < pageSize || currentPage > 50) hasMorePages = false;
                    else currentPage++;
                }
                statusText.textContent = `Traitement ${allSalesData.length} articles...`;
                if (allSalesData.length === 0) throw new Error("Aucune vente trouvée via API.");

                // Process Sales - MODIFIED PRICE CALCULATION
                const groupedSales = allSalesData.reduce((acc, item) => {
                    const billNo = item.billNo;
                    if (!acc[billNo]) {
                         // Heure: Utilisation de operDate
                         let saleDateTime = new Date(); // Fallback
                         if(item.operDate) {
                              const parts = item.operDate.split(/[\s:]+/); // Format "YYYY-MM-DD HH:MM:SS" ou "YYYY MM DD HH:MM:SS"
                              if (parts.length === 6) {
                                  // Recomposer en format ISO-like YYYY-MM-DDTHH:MM:SS
                                  const isoLikeString = `${parts[0]}-${parts[1]}-${parts[2]}T${parts[3]}:${parts[4]}:${parts[5]}`;
                                  const parsedDate = new Date(isoLikeString);
                                  if (!isNaN(parsedDate.getTime())) {
                                      saleDateTime = parsedDate;
                                  } else {
                                     console.warn(`Invalid operDate format '${item.operDate}' for bill ${billNo}. Using current date/time.`);
                                  }
                              } else {
                                  console.warn(`Unexpected operDate format '${item.operDate}' for bill ${billNo}. Using current date/time.`);
                              }
                         }
                        acc[billNo] = { billNo, items: [], totalPrice: 0, saleDate: saleDateTime };
                    }
                    ...
                    if (matchedProduct) {
                        ...
                        const discountPercent = parseFloat(item.discount) || 0; // Discount en pourcentage (ex: 100 = 100% off)

                        if (!isNaN(quantity) && quantity > 0 && !isNaN(price)) {
                             // Calculer le prix final de l'article avec la remise
                             const finalItemPrice = price * (1 - (discountPercent / 100.0));

                            acc[billNo].items.push({
                                id: matchedProduct.id,
                                name: matchedProduct.name,
                                quantity,
                                price: finalItemPrice // Stocker le prix APRÈS remise par article
                            });
                             // Ajouter le prix total de CET article (après remise * quantité) au total de la facture
                             acc[billNo].totalPrice += (finalItemPrice * quantity);
                        } else {
                            console.warn(`Produit ${item.goodsName || 'N/A'} ignoré (fact ${billNo}): qté/prix invalide.`);
                        }
                    } else {
                        console.warn(`Produit API "${item.goodsName || 'N/A'}" (fact ${billNo}) non trouvé.`);
                    }
                    return acc;
                }, {});
                ...
                if (salesToSave.length === 0) throw new Error("Aucune vente valide à importer.");

                // Check Stock & Save Batch
                 const batch = writeBatch(db);
                 const failedStockChecks = [];
                 statusText.textContent = `Vérification stock pour ${salesToSave.length} commandes...`;
                 for (const sale of salesToSave) {
                     const tempDeductions = new Map();
                     let stockAvailable = true;
                     for (const saleItem of sale.items) {
                         const product = allProducts.find(p => p.id === saleItem.id); if (!product) continue;
                         if (product.recipe?.length > 0) {
                             for (const r of product.recipe) tempDeductions.set(r.id, (tempDeductions.get(r.id) || 0) + r.quantity * saleItem.quantity);
                         } else if (product.linkedIngredientId) {
                             const lQty = product.linkedIngredientQuantity || 1;
                             tempDeductions.set(product.linkedIngredientId, (tempDeductions.get(product.linkedIngredientId) || 0) + lQty * saleItem.quantity);
                         }
                     }
                     for (const [ingId, qty] of tempDeductions.entries()) {
                         const ing = allIngredients.find(i => i.id === ingId);
                         if (!ing || ing.quantity < qty) {
                             ...
                             const failName = sale.items.find(i=> allProducts.find(p=>p.id===i.id)?.recipe?.some(r=>r.id===ingId) || allProducts.find(p=>p.id===i.id)?.linkedIngredientId===ingId)?.name ||'?';
                             failedStockChecks.push(`Fact ${sale.billNo} (${failName} -> ${ing?.name||'?'})`);
                             break;
                         }
                     }
                     if (stockAvailable) {
                         for (const [ingId, qty] of tempDeductions.entries()) batch.update(doc(ingredientsCol, ingId), { quantity: increment(-qty) });
                         // Enregistrer la vente avec le totalPrice calculé (somme des prix finaux * qté)
                         batch.set(doc(collection(db, ordersCol.path)), { items: sale.items, totalPrice: sale.totalPrice, createdAt: serverTimestamp(), saleDate: sale.saleDate });
                         salesImportedCount++;
                     } else salesSkippedCount++;
                 }

                // Commit Batch
                 if (salesImportedCount > 0) {
                     statusText.textContent = `Enregistrement ${salesImportedCount} vente(s)...`;
                     await batch.commit();
                     let msg = `${salesImportedCount} vente(s) importée(s).`;
                     if (salesSkippedCount > 0) msg += ` ${salesSkippedCount} ignorée(s) (stock).`;
                     showToast(msg);
                     if(salesSkippedCount > 0) console.warn("Ventes ignorées:", failedStockChecks);
                 } else if (salesSkippedCount > 0) {
                     showToast(`Aucune vente importée. ${salesSkippedCount} ignorée(s) (stock).`);
                     console.warn("Ventes ignorées:", failedStockChecks);
                 } else throw new Error("Aucune vente valide trouvée après traitement.");

            } catch (error) {
                console.error("Erreur Import API:", error);
                let userMsg = `Erreur: ${error.message}`;
                if (error.message.includes('fetch')) userMsg = `Erreur réseau proxy (${PROXY_URL}).`;
                showToast(userMsg);
            } finally {
                loader.classList.add('hidden');
                statusText.textContent = '';
                submitButton.disabled = false;
                if(salesImportedCount > 0 || (salesSkippedCount === 0 && salesImportedCount === 0 && allSalesData.length === 0)) closeModal('api-import-modal');
            }
        });


        // --- IMPORT/EXPORT CSV ---
        // MODIFICATION: Format date CSV (ajout heure)
        const CSV_INSTRUCTIONS = {
            ingredients: `<p>4 colonnes: <strong>nom,quantite,unite,cout</strong></p><pre>nom,quantite,unite,cout\nTomates,10,kg,2.5\n</pre>`,
            products: `<p>4 colonnes: <strong>nom,prix,type,details</strong></p><p>Pizza/Entrée/Dessert: "Ing1:qty1:unit1;Ing2:qty2:unit2"</p><p>Boisson/Autre: "ArticleStock:qtyConsommée"</p><pre>Margherita,8.5,Pizza,"Pâte:1:p;Sauce:0.1:kg"\nCoca,2,Boisson,"Canette Coca:1"</pre>`,
            orders: `<p>3 colonnes: <strong>date_heure,produits,quantites</strong></p><p>Format date_heure: JJ/MM/AAAA HH:MM</p><pre>29/10/2025 14:30,"Margherita;Coca","2;1"</pre>`
        };

        document.getElementById('import-ingredients-btn').addEventListener('click', () => { importType = 'ingredients'; document.getElementById('import-modal-title').textContent = 'Importer Ingrédients'; document.getElementById('import-instructions').innerHTML = CSV_INSTRUCTIONS.ingredients; openModal('import-modal'); });
        document.getElementById('import-products-btn').addEventListener('click', () => { importType = 'products'; document.getElementById('import-modal-title').textContent = 'Importer Produits'; document.getElementById('import-instructions').innerHTML = CSV_INSTRUCTIONS.products; openModal('import-modal'); });
        document.getElementById('import-orders-btn').addEventListener('click', () => { importType = 'orders'; document.getElementById('import-modal-title').textContent = 'Importer Ventes (CSV)'; document.getElementById('import-instructions').innerHTML = CSV_INSTRUCTIONS.orders; openModal('import-modal'); });
        ...
        document.getElementById('process-import-btn').addEventListener('click', async () => {
            const file = document.getElementById('csv-file-input').files[0]; if (!file) return showToast("Choisir un fichier.");
            document.getElementById('process-import-btn').disabled = true;
            const reader = new FileReader();
            reader.onload = async (event) => {
                const lines = event.target.result.split('\n').filter(line => line.trim() !== '');
                if (lines.length <= 1) { showToast("Fichier vide ou en-tête seulement."); closeModal('import-modal'); document.getElementById('csv-file-input').value = ''; document.getElementById('process-import-btn').disabled = false; return; }
                ...
                } catch (error) { console.error("Import error:", error); showToast(`Erreur Import: ${error.message}. Vérifiez ligne correspondante.`);
                } finally { closeModal('import-modal'); document.getElementById('csv-file-input').value = ''; document.getElementById('process-import-btn').disabled = false; }
            };
            reader.readAsText(file);
        });

        async function importIngredients(lines) { /* ... (inchangé) ... */ }
        async function importProducts(lines) { /* ... (inchangé) ... */ }

        // MODIFICATION: Import CSV Ventes avec heure
        async function importOrders(lines) {
            const batch = writeBatch(db);
            for(const [index, line] of lines.entries()) {
                 try {
                    const [dateTimeRaw, pNamesRaw, qtiesRaw] = line.trim().split(',');
                    if(!dateTimeRaw || !pNamesRaw || !qtiesRaw) { console.warn(`Ligne vente ignorée (manque): L${index + 2}`, line); continue; };

                    // Parser "JJ/MM/AAAA HH:MM"
                    const dateTimeParts = dateTimeRaw.trim().match(/^(\d{2})\/(\d{2})\/(\d{4})\s(\d{2}):(\d{2})$/);
                    if (!dateTimeParts) throw new Error(`Format date_heure invalide (JJ/MM/AAAA HH:MM) pour "${dateTimeRaw}"`);
                    const [, day, month, year, hours, minutes] = dateTimeParts;
                    // Construire l'objet Date (mois est 0-indexé)
                    const saleDateTime = new Date(year, month - 1, day, hours, minutes);
                    if (isNaN(saleDateTime.getTime())) throw new Error(`Date/heure invalide: "${dateTimeRaw}"`);

                    const pNames = pNamesRaw.replace(/"/g, '').split(';');
                    const qties = qtiesRaw.replace(/"/g, '').split(';');
                    if (pNames.length !== qties.length) throw new Error(`Nb produits/qtés incohérent`);

                    let items = [], total = 0;
                    let deductions = new Map();
                    for (let i = 0; i < pNames.length; i++) {
                        const name = pNames[i].trim(); const qtyStr = qties[i].trim(); const qty = parseInt(qtyStr);
                        if (isNaN(qty) || qty <= 0) throw new Error(`Qté invalide: ${qtyStr} pour ${name}`);
                        ...
                        total += prod.price * qty; // Le total CSV est basé sur le prix standard, pas de remise ici
                        if (prod.recipe?.length > 0) {
                            for (const r of prod.recipe) deductions.set(r.id, (deductions.get(r.id) || 0) + r.quantity * qty);
                        } else if(prod.linkedIngredientId) {
                            const lQty = prod.linkedIngredientQuantity || 1;
                            deductions.set(prod.linkedIngredientId, (deductions.get(prod.linkedIngredientId) || 0) + lQty * qty);
                        }
                    }
                    for(const [ingId, qtyToDeduct] of deductions.entries()) batch.update(doc(ingredientsCol, ingId), { quantity: increment(-qtyToDeduct) });
                    batch.set(doc(collection(db, ordersCol.path)), { items, totalPrice: total, createdAt: serverTimestamp(), saleDate: saleDateTime }); // Utilise saleDateTime
                 } catch(e) { throw new Error(`Erreur L${index + 2} (ventes): ${e.message}`); }
            }
            await batch.commit();
        }
        ...
        // MODIFICATION: Export CSV Ventes avec heure
        document.getElementById('export-orders-btn').addEventListener('click', () => {
            let csv = "date_heure,produits,quantites,prix_total\n"; // Changement en-tête
            orderHistory.forEach(o => {
                const dateTime = formatDateTimeToDDMMYYYYHHMM(o.saleDate || o.createdAt); // Utilise la fonction avec heure
                const p = o.items.map(i => i.name).join(';');
                const q = o.items.map(i => i.quantity).join(';');
                csv += `${dateTime},"${p}","${q}",${o.totalPrice.toFixed(2)}\n`; // Utilise dateTime
            });
